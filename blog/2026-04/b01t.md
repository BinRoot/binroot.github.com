---
title: Entangled Spaghetti Code
...
The variable `is_backup_needed` feels wrong to me. This imperative approach is anxiety-inducing.
I would prefer an immutable variable since assignment only needs to happen once.
...
This same discourse around type safety, immutable variables, and software engineering best practices is not as mature in the quantum computing world.
...
You can run `CX` in superposition, so it acts on multiple computational basis branches at once and can create entanglement. 
...
However, a major pain in the ass in quantum computing is that temporary variables (ancillae) must be reset before you reuse or discard them.
...
Quantum computing is hard because the set of things you can do with variables (qubits) is a _bit_ limited. 
It would be too easy to simply assign `aux = 0`, pack your bags, and go home.
...
- ✅️ Correctness: every safe quantum program can be expressed by this DSL. 
