...
(function () {
  ...
  document.querySelectorAll('svg.l4-merge').forEach((svg) => {
    ...
    L.text(root, 'rank 0', 136, 118, { size: 24, mono: true, fill: L.PURPLE, weight: 700 });
    L.text(root, 'rank 0', 136, 248, { size: 24, mono: true, fill: L.PURPLE, weight: 700 });
    strip(560, 125, [1, 1, 0]);
    ...
    [[0, 80], [1, 210]].forEach(([m, y]) => {
      L.el('rect', { x: 250, y: y - 20, width: 130, height: 40, rx: 7, fill: '#f3e8ff', stroke: L.PURPLE, 'stroke-width': 1.3 }, fix);
      L.text(fix, `move=${m}`, 315, y, { size: 24, mono: true, weight: 700, fill: L.PURPLE }); });
    const setState = (t) => {
