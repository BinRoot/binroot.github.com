// Published seeded basis-emulation results, distinct from a coherent run.
// The worked trace provides a concrete final board and payoff to check.
(function () {
  const svg = document.getElementById('l3-tested-fig');
  if (!svg) return;
  const L = window.L2;
  const root = L.el('g', {}, svg);
  const X = 60, Y = 62, S = 10;
  ...
  L.text(root, `Our trace: Black ${window.L3.black}, White ${window.L3.white}, payoff ${window.L3.payoff}`, 140, 273, { size: 12, weight: 700 });
  ...
  L.text(root, '1,000 seeded basis runs; 95% interval', (AX0 + AX1) / 2, Y - 20, { size: 14, weight: 700 });
  ...
  const mlab = L.text(root, 'emulated: .281 ± .028', vx(0.281), AY - 34, { size: 13, mono: true, fill: L.BLUE, weight: 700, opacity: 0 });
  ...
  L.text(root, 'Separate checks: preparation amplitudes and scratch restoration', 380, 295, { size: 11.5, fill: L.DIM });
