// The worked center cell has two friends and die 1. Faces 0 and 1 satisfy
// its threshold. These face patterns become joint count/die controls next.
(function () {
  const svg = document.getElementById('l3-compare-fig');
  if (!svg) return;
  const L = window.L2;
  const root = L.el('g', {}, svg);
  const D = window.L3, DIE = D.event.dice[D.focus], C = D.event.friends[D.focus], THR = 4 - C;
  ...
  L.text(root, `c = ${C} friends, threshold 4 − ${C} = ${THR}`, 150, 106, { size: 13 });
  // patterns
  const pats = Array.from({ length: THR }, (_, i) => i).map((v, i) => {
    const y = 140 + i * 40;
    ...
    L.text(g, 'matching face', 306, y, { anchor: 'start', size: 12, fill: L.DIM });
    return { g, v };
  });
  ...
  [0, 1, 2, 3, 4].forEach((c, i) => { const y = TY + 30 + i * 30, thr = 4 - c;
    L.text(root, String(c), TX, y, { size: 14, mono: true, weight: c === C ? 700 : 400 });
    L.text(root, String(thr), TX + 90, y, { size: 14, mono: true, weight: c === C ? 700 : 400 });
    L.text(root, thr ? String(thr) : 'none', TX + 190, y, { size: 14, mono: true, weight: c === C ? 700 : 400, fill: thr ? L.INK : L.DIM }); });
  L.el('rect', { x: TX - 30, y: TY + 30 * (C + 1) - 14, width: 260, height: 28, rx: 6, fill: L.GOLD, opacity: 0.3 }, root);
