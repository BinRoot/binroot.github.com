// l4-qr.js -- Lesson 4, slide 22: continue to Lesson 5.
//
// The Lesson 4 card art, the cliffhanger, the QR code (the same matrix
...
(function () {
  const svg = document.getElementById('qr-l4-fig');
  if (!svg) return;
  const L = window.L2;
  ...
  // art
  const img = L.el('image', { x: 40, y: 40, width: 260, height: 260, href: '../img/myth05.png' }, link);
  img.setAttributeNS(xlink, 'href', '../img/myth05.png');
  L.text(link, 'Lesson 5: Garbage collection', 170, 322, { size: 16, weight: 700 });
  // cliffhanger
  L.text(link, 'Continue to Lesson 5', 540, 44, { size: 26, weight: 700 });
  L.text(link, '“Which of the 155 records', 540, 80, { size: 15, fill: L.ORANGE, weight: 700, italic: true });
  L.text(link, 'could you return early?”', 540, 102, { size: 15, fill: L.ORANGE, weight: 700, italic: true });
  if (QR && QR.d) {
